...
void Des(unsigned char *input,unsigned char *output,unsigned char *key,int mode)
{
	if((input == NULL) || (output == NULL) || (key == NULL))
		return;
	if(mode!=CurrentMode || memcmp(key,CurrentKey,8)){
		CurrentMode=mode;
		memcpy(CurrentKey,key,8);
		deskey(CurrentKey,CurrentMode);
	}
	s_des(input, output);
}
...
void DES(unsigned char *dat,unsigned char *key,int mode)
{
     unsigned char dataout[8];
     Des(dat,dataout,key,mode);
     memcpy(dat,dataout,8);
}


// 1=Encrypt 0=Decrypt
void DES3_16(unsigned char *dat,unsigned char *key,int mode)
{
     if(mode)
     {
          DES(dat,key,1);
...
// 1=Encrypt 0=Decrypt
void DES3_24(unsigned char *dat,unsigned char *key,int mode)
{
     if(mode)
     {
          DES(dat,key,1);
